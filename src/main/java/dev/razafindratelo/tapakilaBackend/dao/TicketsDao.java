...
public class TicketsDao {
    ...
    public long findLastTicketNumber(String eventId) {
        Connection connection = dataSource.getConnection(TicketsDao.class.getName());

        return findLastTicketNumber(connection, eventId);
    }
    ...
    public Tickets save(Tickets ticket) {
        ...
        try (PreparedStatement saveStmt = connection.prepareStatement(sql)) {
            ...
            if (i > 0)
                return findByTicketsIdWithConnection(connection, ticket.getId());
    ...
    public Tickets findByTicketsId(String ticketsId) {
        Connection connection = dataSource.getConnection(TicketsDao.class.getName());

        return findByTicketsIdWithConnection(connection, ticketsId);
    }

    public Tickets findByTicketsIdWithConnection(Connection con, String ticketsId) {
        String sql =
                """
                WITH TicketInfo AS (
                            SELECT
                        ...
                        SELECT
                            ...
                            JSONB_BUILD_OBJECT(
								...
								'associatedEventId', tp.associated_event_id
                            ) AS ticket_info,
                            JSONB_BUILD_OBJECT(
                                'email', ui.email,
								...
								'favoriteEventCategories', ui.user_top_5_categories
                            ) AS purchased_by,
                            ti.associated_event_id AS associated_event_id,
                            JSONB_BUILD_OBJECT(
        ...
        try (PreparedStatement findByIdStmt = con.prepareStatement(sql)) {
            findByIdStmt.setString(1, ticketsId);
			 
            ResultSet rs = findByIdStmt.executeQuery();


            if (rs.next())
                return ticketsMapper.mapFrom(rs);

            throw new ResourceNotFoundException("TIcketsDao.findByTIcketsID :: no tickets with " + ticketsId);
        } catch (SQLException e) {
            throw new RuntimeException("TicketsDao.findByTicketsId :: " + e);
        }
    }
    ...
    public List<Tickets> findAllByUserEmailAndEventId(String userEmail, String eventId, Connection connection) {
        String sql =
                """
						...
						UserInfo AS (
							...
							FROM "user" u
							LEFT JOIN Top5Categories AS t5c ON u.email = t5c.user_email
							GROUP BY u.email
						)
						SELECT
							ti.ticket_id AS id,
						...
						FROM TicketInfo ti
						LEFT JOIN TicketPriceInfo tp ON ti.id_ticket_price = tp.ticket_price_id
						LEFT JOIN UserInfo ui ON ti.user_email = ui.email 
						LEFT JOIN PaymentModeInfo pm ON ti.id_payment_mode = pm.payment_mode_id
                        WHERE ui.email = ? and ti.associated_event_id = ?
                """;
        ...
        try (PreparedStatement findAllByUserStmt = connection.prepareStatement(sql)) {
            ...
            findAllByUserStmt.setString(2, eventId.trim());
	...
	public Set<PrimitiveEventTicketDto> getAllEventRelatedToUserTicket(String userEmail, long page, long size) {
		String sql =
				"""
						SELECT DISTINCT tp.id_event
						FROM ticket t
						LEFT JOIN ticket_price tp ON tp.id = t.id_ticket_price
						LIMIT ? OFFSET ?
				""";

		Set<PrimitiveEventTicketDto> results = new HashSet<>();
		Connection connection = dataSource.getConnection(TicketsDao.class.getName());

		try (PreparedStatement stmt = connection.prepareStatement(sql)) {

			stmt.setLong(1, page);
			stmt.setLong(2, size * (page - 1));
			ResultSet rs = stmt.executeQuery();

			while (rs.next()) {
				String id = rs.getString(1);
				List<Tickets> tps = findAllByUserEmailAndEventId(userEmail, id, connection);
